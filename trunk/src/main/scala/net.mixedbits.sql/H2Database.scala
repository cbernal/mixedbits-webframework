...
case class H2Database(datasource:DataSource,schemaName:String) extends SqlDatabase{
  ...
  def createTable(table:SqlTable)(implicit connection:SqlWriteConnection) = {
    ...
    table.execute("CREATE TABLE `%1$s`.`%2$s` ( %3$s );".format( schemaName, table.name, tableDefinition))
  }

  override def exists(table:SqlTable)(implicit connection:SqlConnection) =
    connection.rawConnection.getMetaData().getTables(connection.database.schemaName.toUpperCase,null,table.name.toUpperCase,null).first()
  
  override def existingColumnNames(table:SqlTable)(implicit connection:SqlConnection):Seq[String] = 
    (for(column <- connection.rawConnection.getMetaData().getColumns(connection.database.schemaName.toUpperCase,null,table.name.toUpperCase,null))
